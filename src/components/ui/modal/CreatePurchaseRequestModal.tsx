...
const CreatePurchaseRequest: React.FC<CreatePurchaseRequestModalProps> = ({ isOpen, onClose, onSave, initialData }) => {
    ...
    const [subCostCenter, setSubCostCenter] = useState(initialData?.subCostCenter || "");
    ...
    const [items, setItems] = useState<ItemDetail[]>(
        initialData?.items && initialData.items.length > 0
            ? initialData.items.map((item, idx) => ({ ...item, line: idx + 1 }))
            : [
                {
                    line: 1,
                    description: "",
            ...
            <div className="flex flex-col gap-6 overflow-y-auto max-h-[80vh] custom-scrollbar">
                <div>
                    <h2 className="text-xl sm:text-2xl font-semibold text-gray-800 dark:text-white">
                        {initialData ? "Edit Purchase Request" : "Create Purchase Request"}
                    </h2>
                    <p className="text-sm text-gray-500 dark:text-gray-400">
                        {initialData ? "Editing purchase request." : "Fill in the details for the new purchase request."}
                    </p>
                </div>
                ...
                <div className="flex flex-wrap justify-end gap-2 mt-4">
                    <Button size="sm" variant="outline" onClick={onClose} className="px-4 py-1.5 border border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-md transition text-sm">Cancel</Button>
