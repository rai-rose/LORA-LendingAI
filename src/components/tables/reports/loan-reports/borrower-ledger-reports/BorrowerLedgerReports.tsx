...
const borrowersLedgerReportsData: Report[] = [
  { id: 1, code: "R6L01", module: "Borrower Transaction History Report" },
  { id: 2, code: "R6L02", module: "Borrower Balance Summary Report" },
  { id: 3, code: "R6L03", module: "Borrower Payment Schedule Report" },
  { id: 4, code: "R6L04", module: "Borrower Delinquency Report" },
  { id: 5, code: "R6L05", module: "Borrower Account Status Report" },
];

export default function BorrowersLedgerReportsTable() {
  ...
  const paginatedData = useMemo(() => {
    ...
    return borrowersLedgerReportsData.slice(startIndex, endIndex);
  }, [currentPage]);

  const handleReportClick = (module: string) => {
    const formattedModuleName = module.toLowerCase().replace(/\s+/g, "-");
    router.push(`/reports/borrowers-ledger/${formattedModuleName}`);
  };

  const handleBack = () => {
    router.push("/reports/loan-reports");
  };
  ...
  return (
    <div className="space-y-4">
      ...
      <Pagination
        ...
        totalItems={borrowersLedgerReportsData.length}
